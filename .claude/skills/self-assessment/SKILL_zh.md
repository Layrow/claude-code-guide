...
version: 2.2.0
description: 全面的 Claude Code 自我评估与学习路径顾问。它会运行一个覆盖 10 个特性领域的多类别测验，输出按主题拆分的技能画像，识别具体缺口，并生成带优先级的个性化学习路径。适用于用户提出 “assess my level”、“take the quiz”、“find my level”、“where should I start”、“what should I learn next”、“check my skills”、“skill check” 或 “level up” 这类请求时。
---

# Self-Assessment & Learning Path Advisor

这是一个全面的交互式评估，会衡量你在 10 个特性领域里的 Claude Code 熟练度，找出具体技能缺口，并生成个性化升级路径。
...
"第 1/2 部分：以下这些 Claude Code 能力，你已经具备哪些？"
Options:
1. "Start Claude Code and chat" — 我可以运行 `claude` 并与它交互
2. "Created/edited CLAUDE.md" — 我已经配置过项目级或用户级记忆
...
2. "Set up hooks" — 在 ~/.claude/settings.json 中配置过 hooks
3. "Created/used subagents" — 使用过 .claude/agents/ 做任务委派
4. "Used print mode (claude -p)" — 使用过 `claude -p` 做无交互或 CI/CD 场景
...
4. "Used memory hierarchy features" — 理解 7 层优先级，使用过 .claude/rules/ 目录、路径限定规则或 @import 语法
...
2. "Used advanced MCP features" — 用过项目级 .mcp.json、OAuth 认证、带 @mentions 的 MCP resources、Tool Search，或 `claude mcp serve`
3. "Created or configured custom subagents" — 在 .claude/agents/ 中定义过带自定义 tools、model 或 permissions 的 agents
...
4. "Used remote/desktop/web features" — 使用过 `claude remote-control`、`claude --remote`、`/teleport`、`/desktop` 或 `claude -w`
...
"以下哪些事情你做过？可多选。"
Options:
1. "Installed or created a plugin" — 使用过 marketplace 中的插件，或创建过带 plugin.json manifest 的 .claude-plugin/ 目录
...
3. "Used print mode in scripts or CI/CD" — 在脚本或 CI/CD 中使用过 `claude -p`、`--output-format json`、`--max-turns`、管道输入或 GitHub Actions 集成
...
- Focus on: CLAUDE.md 创建、`/init` 和 `/memory` 命令、使用 `#` 前缀快速更新
- Key exercise: 创建一个记录编码规范的项目级 CLAUDE.md
- Done when: Claude 能在跨会话中记住你的偏好

**Memory (score 1 — review)**:
- Focus on: 7 层优先级体系、带路径限定的 .claude/rules/ 目录、`@import` 语法（最大深度 5）、Auto Memory 的 MEMORY.md（200 行限制）
...
- Focus on: `claude mcp add` 命令、传输类型（推荐 HTTP）、GitHub MCP 配置、环境变量展开
- Key exercise: 添加 GitHub MCP server 并查询 PR
- Done when: 你能通过 MCP 查询外部服务的实时数据

**MCP (score 1 — review)**:
- Focus on: 项目级 .mcp.json（需要团队批准）、OAuth 2.0、`@server:resource` 形式的 MCP resources、Tool Search（ENABLE_TOOL_SEARCH）、`claude mcp serve`、输出限制（10k / 25k / 50k）
- Done when: 你已经配置好项目级 .mcp.json，并理解 Tool Search 自动模式

**Subagents (score 0)**:
- Tutorial: [04-subagents/](../../../04-subagents/)
- Focus on: agent 文件格式（.claude/agents/*.md）、内置 agents（general-purpose、Plan、Explore）、tools / model / permissionMode 配置
- Key exercise: 创建一个 code-reviewer subagent 并测试委派
- Done when: Claude 会把代码审查任务委派给你的自定义 agent
...
- Focus on: Remote control（`claude remote-control`）、web sessions（`claude --remote`）、desktop handoff（`/desktop`）、worktrees（`claude -w`）、task lists（Ctrl+T）、企业级 managed settings
- Done when: 你能在 CLI、Web 和桌面端之间顺畅交接会话

**Plugins (score 0)**:
- Tutorial: [07-plugins/](../../../07-plugins/)
- Focus on: 插件结构（.claude-plugin/plugin.json）、插件能打包什么（commands、agents、MCP、hooks、settings）、从 marketplace 安装
...
- Focus on: 交互模式 vs print mode、带管道的 `claude -p`、`--output-format json`、会话管理（-c / -r）
- Key exercise: 把一个文件管道给 `claude -p`，并拿到 JSON 输出
- Done when: 你能在脚本中以无交互方式使用 Claude

**CLI (score 1 — review)**:
- Focus on: 带 JSON 配置的 --agents、结构化输出的 --json-schema、--fallback-model、--from-pr、--strict-mcp-config、for 循环批处理、`claude mcp serve`
- Done when: 你已经有一个使用 Claude 并输出结构化 JSON 的 CI/CD 脚本
