...
这个文件为 Codex（Codex.ai/code）在本仓库中工作时提供指导。

## 项目概览

Codex How To 是一个面向 Codex 功能的教程仓库。这是一个 **documentation-as-code** 项目，主要产出是按编号组织的 Markdown 学习模块，而不是一个可执行应用。

**架构**：每个模块（01-10）聚焦一个特定的 Codex 特性，包含可直接复制使用的模板、Mermaid 图和示例。构建系统会校验文档质量，并生成 EPUB 电子书。
...
1. **编号目录表示学习顺序** — `01-10` 前缀代表推荐的 Codex 学习路径。这个编号是有意设计的，不要按字母顺序重排。
