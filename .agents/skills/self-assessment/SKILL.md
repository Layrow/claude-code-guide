...
version: 2.2.0
description: Comprehensive Codex self-assessment and learning path advisor. Runs a multi-category quiz covering 10 feature areas, produces a detailed skill profile with per-topic scores, identifies specific gaps, and generates a personalized learning path with prioritized next steps. Use when asked to "assess my level", "take the quiz", "find my level", "where should I start", "what should I learn next", "check my skills", "skill check", or "level up".
---

# Self-Assessment & Learning Path Advisor

Comprehensive interactive assessment that evaluates Codex proficiency across 10 feature areas, identifies specific skill gaps, and generates a personalized learning path to level up.
...
"Part 1/2: Which of these Codex skills do you already have?"
Options:
1. "Start Codex and chat" — I can run `Codex` and interact with it
2. "Created/edited AGENTS.md" — I have set up project or user memory
...
2. "Set up hooks" — Configured hooks in ~/.Codex/settings.json
3. "Created/used subagents" — Used .Codex/agents/ for task delegation
4. "Used print mode (Codex -p)" — Used `Codex -p` for non-interactive or CI/CD use
...
Options:
1. "Created a custom slash command or skill" — Written a SKILL.md file with frontmatter, or created .Codex/commands/ files
2. "Used dynamic context in commands" — Used `$ARGUMENTS`, `$0`/`$1`, backtick `!command` syntax, or `@file` references in skill/command files
3. "Set up project + personal memory" — Created both a project AGENTS.md and personal ~/.Codex/AGENTS.md (or Codex.local.md)
4. "Used memory hierarchy features" — Understand the 7-level priority order, used .Codex/rules/ directory, path-specific rules, or @import syntax
...
2. "Used advanced MCP features" — Project-scope .mcp.json, OAuth authentication, MCP resources with @mentions, Tool Search, or `Codex mcp serve`
3. "Created or configured custom subagents" — Defined agents in .Codex/agents/ with custom tools, model, or permissions
...
4. "Used remote/desktop/web features" — Used `Codex remote-control`, `Codex --remote`, `/teleport`, `/desktop`, or worktrees with `Codex -w`
...
"Which of these have you done? Select all that apply."
Options:
1. "Installed or created a plugin" — Used a bundled plugin from marketplace, or created a .Codex-plugin/ directory with plugin.json manifest
...
3. "Used print mode in scripts or CI/CD" — Used `Codex -p` with --output-format json, --max-turns, piped input, or integrated into GitHub Actions / CI pipelines
...
- Focus on: AGENTS.md creation, `/init` and `/memory` commands, `#` prefix for quick updates
- Key exercise: Create a project AGENTS.md with your coding standards
- Done when: Codex remembers your preferences across sessions

**Memory (score 1 — review)**:
- Focus on: 7-level hierarchy and priority order, .Codex/rules/ directory with path-specific rules, `@import` syntax (max depth 5), Auto Memory MEMORY.md (200-line limit)
...
- Focus on: `Codex mcp add` command, transport types (HTTP recommended), GitHub MCP setup, environment variable expansion
- Key exercise: Add GitHub MCP server and query PRs
- Done when: You can query live data from an external service via MCP

**MCP (score 1 — review)**:
- Focus on: Project-scope .mcp.json (requires team approval), OAuth 2.0 auth, MCP resources with `@server:resource` mentions, Tool Search (ENABLE_TOOL_SEARCH), `Codex mcp serve`, output limits (10k/25k/50k)
- Done when: You have a project .mcp.json and understand Tool Search auto mode

**Subagents (score 0)**:
- Tutorial: [04-subagents/](../../../04-subagents/)
- Focus on: Agent file format (.Codex/agents/*.md), built-in agents (general-purpose, Plan, Explore), tools/model/permissionMode config
- Key exercise: Create a code-reviewer subagent and test delegation
- Done when: Codex delegates code review to your custom agent
...
- Focus on: Remote control (`Codex remote-control`), web sessions (`Codex --remote`), desktop handoff (`/desktop`), worktrees (`Codex -w`), task lists (Ctrl+T), managed settings for enterprise
- Done when: You can hand off sessions between CLI, web, and desktop

**Plugins (score 0)**:
- Tutorial: [07-plugins/](../../../07-plugins/)
- Focus on: Plugin structure (.Codex-plugin/plugin.json), what plugins bundle (commands, agents, MCP, hooks, settings), installation from marketplace
...
- Focus on: Interactive vs print mode, `Codex -p` with piping, `--output-format json`, session management (-c/-r)
- Key exercise: Pipe a file to `Codex -p` and get JSON output
- Done when: You can use Codex non-interactively in a script

**CLI (score 1 — review)**:
- Focus on: --agents flag with JSON config, --json-schema for structured output, --fallback-model, --from-pr, --strict-mcp-config, batch processing with for loops, `Codex mcp serve`
- Done when: You have a CI/CD script that uses Codex with structured JSON output
